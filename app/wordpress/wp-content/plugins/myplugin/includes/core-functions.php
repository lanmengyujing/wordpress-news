...
function myplugin_login_enqueue_styles(){
	...
	wp_enqueue_script( 
		...
		plugin_dir_url(dirname(__FILE__)) . 'public/css/myplugin-login.js', 
		array(),
		false,
		true
	);
...
function myplugin_custom_login_styles() {
	...
	if ( isset( $options['custom_style'] ) && ! empty( $options['custom_style'] ) ) {
		...
		$should_show_custom_style = sanitize_text_field($options['custom_style'])==="enable";
        if( $should_show_custom_style ){
            myplugin_login_enqueue_styles();
        }
	}
}
